...
public final class RatPoly {
    ...
    /**
     * Returns the size of this RatPoly i.e. how many terms there are.
     *
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        ...
        for (int i = 0; i < lst.size(); i++) {
            lst.set(i, new RatTerm(scalar.mul(lst.get(i).getCoeff()), lst.get(i).getExpt()));
        }
    }
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        for (int i = 0; i < lst.size(); i++) {
            ...
            if (newTerm.getExpt() == lst.get(i).getExpt()) {
                RatTerm t = lst.get(i).add(newTerm);
                if (t.getCoeff().equals(RatNum.ZERO)) {
                    lst.remove(i);
                    i--;
                    return;
                }
    ...
    /**
     * Return a mutable copy of the RatPoly
     *
     * @return an ArrayList equal to this; if this.isNaN(), returns some r such that r.isNaN()
     */
    public ArrayList<RatTerm> copy() {
        ArrayList<RatTerm> result = new ArrayList<RatTerm>();
        for (int i = 0; i < terms.size(); i++) {
            result.add(new RatTerm(terms.get(i).getCoeff(), terms.get(i).getExpt()));
        }
        return result;
    }
    ...
    public RatPoly negate() {
        ...
        // since RatPoly is immutable, needs to make a copy
        ArrayList<RatTerm> copy = this.copy();
        // scaleCoeff requires us to pass a mutable list
    ...
    public RatPoly mul(RatPoly p) {
        ...
        for (RatTerm t : terms) {
            ...
            // term * polynomial p
            scaleCoeff(copy, t.getCoeff());
            incremExpt(copy, t.getExpt());
            r = r.add(new RatPoly(copy));
        }
        r.checkRep();
        return r;
    }
    ...
    public RatPoly div(RatPoly p) {
        if (p.size() == 0 || this.isNaN() || p.isNaN()) {
            return NaN;
        }
        ...
        RatPoly r = ZERO;
        RatPoly temp = new RatPoly(this.copy());
        while(temp.size() != 0 && temp.degree() >= p.degree()) {
            RatTerm tempHighest = temp.getTerm(temp.degree());
            RatTerm pHighest = p.getTerm(p.degree());
            RatTerm i = tempHighest.div(pHighest);
            RatPoly t = new RatPoly(i);
            r = r.add(t);
            RatPoly s = t.mul(p);
            temp = temp.sub(s);
        }
    ...
    public double eval(double d) {
        ...
        double r = 0;
        // evaluate each term in terms at d
        for (RatTerm t : terms) {
            r += t.eval(d);
        }
        return r;
